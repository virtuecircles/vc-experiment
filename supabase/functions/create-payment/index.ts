...
serve(async (req) => {
  ...
  try {
    ...
    logStep("Received request", { eventId, eventTitle, amount });

    if (!eventTitle || !amount) {
      throw new Error("Missing required fields: eventTitle and amount");
    }
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      mode: "payment",
      success_url: `${req.headers.get("origin")}/events?payment=success&event=${eventId}`,
      cancel_url: `${req.headers.get("origin")}/events?payment=canceled`,
      metadata: {
        event_id: eventId || "",
        user_id: user.id,
      },
